...
| `finnhub` | quotes (+**websocket live trades** = only true REALTIME), fundamentals, insiders, ratings, earnings, earnings calendar, IPO calendar | free key | NEAR-RT (REALTIME via ws) | 50/min (limit 60/min) | US-focused. WS: smart LRU subscriptions from on-screen symbols, cap 40, idle unsub 2 min, backoff reconnect. Price targets + short interest are premium — not offered. **Live-verified 2026-07-17** (incl. ws connect/subscribe/idle-close). |
| `marketaux` | news (entity tagging + sentiment) | free key | NEAR-RT | 10/min, 90/day (limit 100/day, 3 articles/resp) | Sentiment per entity −1..1. Cached hard (news TTL 5 min) to live inside the tiny quota. **Live-verified 2026-07-17.** |
| `fred` | calendar (US release schedule) | free key | OFFICIAL | 60/min | fred/releases/dates, importance-ranked majors (CPI/NFP/GDP/FOMC=high). US only. First call after boot can be slow (>10s) — the cache + stale-serve absorb it. **Live-verified 2026-07-17.** |
